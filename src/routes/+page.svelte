<script lang="ts">
	...
	function construct_chat_history(chatMessages: ChatCompletionRequestMessage[]) {
		let history = 'Assistant: Hi. I’ve had a tough time recently and I wanted to talk to someone about it. '
	...
	const f_handleSubmit = async () => {
		f_loading = true
		f_query = `The following is a ongoing conversation between me and my friend: ${construct_chat_history(chatMessages)}
		Here is my message that I'm planning on sending next: ${f_query}
		...
		`
		f_chatMessages = [...f_chatMessages, { role: 'user', content: f_query }]
...
<div class="flex">
	<div class="flex flex-col pt-4 w-[500px] px-8 gap-2 my-8 -ml-64">
		<h1 class="text-2xl font-bold w-full">Conversation Hint</h1>
...
<div class="flex flex-col w-[300px] px-8">
	<div class="flex flex-col pt-4 w-[500px] px-8 bg-white rounded-md my-8 gap-2">
		...
		>
			<input type="text" class="input input-bordered w-full" bind:value={f_query} />
